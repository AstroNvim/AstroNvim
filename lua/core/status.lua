...
function M.hl.mode()
  return { fg = M.modes[vim.fn.mode()][2] }
end

function M.hl.mode_fg()
...
function M.provider.fill()
  return "%="
end

function M.provider.percentage(opts)
  return M.utils.stylize("%p%%", opts)
end
...
function M.provider.filetype(opts)
  return function()
    return M.utils.stylize(string.lower(vim.bo.filetype), opts)
  end
end

function M.provider.filename(modify, opts)
  return function()
    return M.utils.stylize(vim.fn.fnamemodify(vim.api.nvim_buf_get_name(0), modify or ":t"), opts)
  end
end
...
function M.provider.git_branch(opts)
  return function()
    return M.utils.stylize(vim.b.gitsigns_head or "", opts)
  end
end
...
function M.provider.treesitter_status(opts)
  return function()
    ...
    return M.utils.stylize((ts and next(ts)) and "TS" or "", opts)
  end
end

function M.provider.str(str, opts)
  return M.utils.stylize(str or " ", opts)
end

function M.condition.git_available()

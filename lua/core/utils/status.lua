...
astronvim.status.env.icon_highlight = astronvim.user_plugin_opts("heirline.icon_highlight", {
  file_icon = {
    tabline = true,
    statusline = true,
  },
...
function astronvim.status.init.breadcrumbs(opts)
  opts = astronvim.default_tbl(opts, {
    separator = " > ",
    icon = { enabled = true, hl = astronvim.status.env.icon_highlight.breadcrumbs },
...
function astronvim.status.component.file_info(opts)
  opts = astronvim.default_tbl(opts, {
    file_icon = {
      hl = function(self)
        local hl_enabled = astronvim.status.env.icon_highlight.file_icon.statusline
...
function astronvim.status.component.tabline_file_info(opts)
  return astronvim.status.component.file_info(astronvim.default_tbl(opts, {
    file_icon = {
      ...
      hl = function(self)
        local hl_enabled = astronvim.status.env.icon_highlight.file_icon.tabline
        if type(hl_enabled) == "function" then hl_enabled = hl_enabled(self) end
        if hl_enabled then return astronvim.status.hl.filetype_color(self) end
      end,
    },

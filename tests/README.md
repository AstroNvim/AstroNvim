...
Run `make test-fingerprint` to print the single SHA-256 compatibility fingerprint used by shared CI cache keys. It does not prepare or inspect `.tests/`. The fingerprint binds the harness schema, managed layout, explicit test dependencies, and AstroNvim plugin specification sources that control the resolved dependency graph.
